...
public:
    ...
    explicit websocket_sink(std::shared_ptr<webSocketControl> g_webSocketControl)
    {
...
protected:
    ...
    std::shared_ptr<webSocketControl> g_webSocketControl;
};
...
inline std::shared_ptr<logger> websocket_logger_mt(const std::string &logger_name, std::shared_ptr<webSocketControl> g_webSocketControl)
{
    return Factory::template create<sinks::websocket_sink_mt>(logger_name, g_webSocketControl);
}
